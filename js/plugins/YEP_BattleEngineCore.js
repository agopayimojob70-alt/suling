...
Game_Action.prototype.applyGlobal = function() {
    ...
    // 修改 当处于战斗时(此项目中为卡牌战斗时，可以触发数据库单位【效果】的applyGlobal)
    if ($gameParty.inBattle() && !(SceneManager._scene instanceof Scene_Battle)) return;
    Yanfly.BEC.Game_Action_applyGlobal.call(this);
};

//## 修改 加入一个新interpreter处理战斗时公共事件
//不可直接用$gameTroop的inter，否则其setupReservedCommonEvent会覆盖敌群事件页
Yanfly.BEC.BattleManager_updateEventMain = BattleManager.updateEventMain;
BattleManager.updateEventMain = function() {
    let r = Yanfly.BEC.BattleManager_updateEventMain.call(this, ...arguments);
    if($gameTroop && $gameTroop._tmp_extra_cev_inter){
      let i = $gameTroop._tmp_extra_cev_inter;
      i.setupReservedCommonEvent(); 
      i.update();
    }
    return r;
};
Yanfly.BEC.Game_Troop_clear_419 = Game_Troop.prototype.clear;
Game_Troop.prototype.clear = function() {
  Yanfly.BEC.Game_Troop_clear_419.call(this, ...arguments);
    this._tmp_extra_cev_inter = new Game_Interpreter();
};
//##
